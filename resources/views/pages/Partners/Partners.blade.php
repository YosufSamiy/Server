...
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0">{{ trans('sections.Partners') }}</h4>
        </div>
        <div class="col-sm-6">
...
<div class="row">   
    ...
    <div class="col-xl-12 mb-30">     
      <div class="card card-statistics h-100"> 
        <div class="card-body">
            <a href="{{route('Partners.create')}}" type="button" class="btn btn-success"> <i class="fas fa-plus"></i>&nbsp;	{{ trans('sections.add_partners') }}</a>
            <br>
            <br>
          ...
          <table id="datatable" class="table table-striped table-bordered p-0">
            <thead>
                <tr>
                    <th>#</th>
                    <th>{{ trans('sections.name_Partners') }}</th>
                    <th>{{ trans('sections.topic_Partners') }}</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$loop->index}}</td>
                    <td>{{$Partners->name}}</td>
                    <td>{{$Partners->topic}}</td>
                    <td>
                         <form action="{{route('Partners.destroy',$Partners->id)}}" method="POST">
                            @csrf
                            @method('delete')

                        <a type="submit" href="{{route('Partners.edit',$Partners->id)}}" ) class="btn btn-info btn-sm" ><i class="fa fa-edit"></i></a>
